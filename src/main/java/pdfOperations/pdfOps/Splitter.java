 ...
 * Splitter
 */
public class Splitter extends basicOps{

    public static void splitPDF(File f, int from) throws IOException{
        ...
        if(from == 0){
            for(int i = 0;i<num_pages;i++){
                ...
                basicOps.getWriteFile(new String[]{file_path.substring(0, file_path.length()-4) + "_" + (i+1) + ".pdf"});
            ...
            for(int i = 0;i<from -1 ;i++){
                basicOps.getWriteFile(new String[]{file_path.substring(0, file_path.length()-4) + "_" + 1 + ".pdf"});
